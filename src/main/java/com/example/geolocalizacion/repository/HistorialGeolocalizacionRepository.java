package com.example.geolocalizacion.repository;

import com.example.geolocalizacion.model.HistorialGeolocalizacion;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;
import java.util.List;

@Repository
public interface HistorialGeolocalizacionRepository extends JpaRepository<HistorialGeolocalizacion, Long> {

    List<HistorialGeolocalizacion> findTop20ByRunPacienteOrderByFechaDesc(String runPaciente);

    List<HistorialGeolocalizacion> findByRunPacienteOrderByFechaDesc(String runPaciente, Pageable pageable);
}